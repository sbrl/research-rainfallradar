...
def parse_item(metadata, output_size=100, input_size="same", water_threshold=0.1, water_bins=2, heightmap=None, rainfall_scale_up=1, do_remove_isolated_pixels=True):
	...
	print(
		...
		"/",
		f"w {rainfall_width_source} h {rainfall_height_source}",
	)
	...
	def parse_item_inner(item):
		...
		rainfall = tf.reshape(
			rainfall, tf.constant(metadata["rainfallradar"], dtype=tf.int32)
		)
		...
		if rainfall_scale_up > 1:
			rainfall = tf.repeat(
				tf.repeat(rainfall, rainfall_scale_up, axis=0),
				rainfall_scale_up,
				axis=1,
			)
		if input_size is not None:
			rainfall = tf.image.crop_to_bounding_box(
				rainfall,
		...
		water = tf.expand_dims(
			water, axis=-1
		)  # [height, width] → [height, width, channels=1]
		water = tf.image.crop_to_bounding_box(
			water,
		...
		print(
			"DEBUG:dataset BEFORE_SQUEEZE water", water.shape
		)  #  [ height, width, channel=1 ]
		water = tf.squeeze(water)
		print("DEBUG:dataset AFTER_SQUEEZE water", water.shape)  # [ height, width ]
		...
		if water_threshold is None:
			...
			# This is important because DeepLabV3+ MUST output as [ height, width, classes ] and since we're not in classification mode then we can't use SparseCategoricalCrossentropy, which corrects [ height, width ] → [ height, width, channels ] via one-hot automagically.
